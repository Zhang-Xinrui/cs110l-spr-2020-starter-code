...
use std::env;
use std::fs::{File, read_dir}; // For read_file_lines()
...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..seq1.len() {
        for j in 0..seq2.len() {
            if (seq1[i] == seq2[j]) {
                grid.set(i + 1, j + 1, 1 + grid.get(i, j).unwrap());
            } else if (grid.get(i + 1, j).unwrap() > grid.get(i, j + 1).unwrap()) {
                grid.set(i + 1, j + 1, grid.get(i + 1, j).unwrap());
            } else {
                grid.set(i + 1, j + 1, grid.get(i, j + 1).unwrap());
            }
        }
    }
    return grid;
...
fn main() {
    ...
    let file1 = read_file_lines(filename1).expect("First file path invalid");
    let file2 = read_file_lines(filename2).expect("Second file path invalid");
